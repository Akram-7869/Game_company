...
exports.getToken = asyncHandler(async (req, res, next) => {
    ...
    const row = await Setting.findOne({ type: 'PAYMENT', name: 'PHONEPAY' });
    if (amount <= 0) {
        return next(
    ...
    let data = {
        "merchantId": row.one.MERCHANTID,
        "merchantTransactionId": tran._id,
        "merchantUserId": req.player._id,
        "amount": amount * 100,
        "redirectUrl": process.env.APP_URI + '/success/phonepay',
        "redirectMode": "REDIRECT",
        "callbackUrl": process.env.API_URI + '/payments/phonepaypg/webhook',
        "mobileNumber": req.player.phone,
        "paymentInstrument": {
            "type": "PAY_PAGE"
        }
    }

    let base64data = new Buffer.from(JSON.stringify(data)).toString("base64");
    let xheader = base64data + "/pg/v1/pay";
    let finalXheader = calculateXHash(xheader, row);
    //let urlpg = 'https://api.phonepe.com/apis/hermes/pg/v1/pay';
    let gatewayurl = 'https://api-preprod.phonepe.com/apis/pg-sandbox/pg/v1/pay';
    if (row.one.mode === 'production') {
        gatewayurl = 'https://api.phonepe.com/apis/hermes/pg/v1/pay';
    }
    let urlpg = row.one.URL;
    const options = {
        method: 'POST',
        url: gatewayurl,
        headers: { accept: 'application/json', 'Content-Type': 'application/json', 'X-VERIFY': finalXheader },
...
exports.handleNotify = asyncHandler(async (req, res, next) => {
    console.log('phonepay-notify-body');
    let { response } = req.body;
    let responsString = Buffer.from(response, 'base64').toString('utf-8');
    responsObj = JSON.parse(responsString);
    console.log('phonepay-notify', responsObj);

    const orderId = responsObj.data.merchantTransactionId;
    const amount = responsObj.data.amount * 0.01;
    // let  xverify = req.headers['x-verify']; 
    // let calHash =calculateXHash(response);
    // if(xverify !== calHash){
    //   return next(
